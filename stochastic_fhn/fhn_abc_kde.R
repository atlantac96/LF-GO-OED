  ...
  fitzhugh_nagumo<- function(time, y0, parms) {
    ...
    with(as.list(parms), {
      for (i in 1:n) {
        ...
        # or use rwiener(end = 1, frequency = 1000) with e1070 package
...
for(replicate_num in 1:num_replicates){
  ...
  for(k in 2:length(zetarange)){
    ...
    rat_list[[k]] <- foreach(i = 1:nrow(cc), .combine = cbind) %dopar% {
      ...
      library(abc)
      start<-Sys.time()
      cv.nn<-cv4abc(cc[,1:2], cc[,3:4], nval=800,
      ...
      if (nrow(check$adj.values) != 0) {
        ...
        set.seed(56)
        kde1<-kde(check$adj.values, density=TRUE)
        kde1p<- predict(kde1, x=pp)
        kde2<-kde(zi, density=TRUE)
        kde2p<- predict(kde2, x=pp)
        kde1p/kde2p
...
save(ut_matrix, file="stochasticfhnparaminf.RDa") # for uspike 0.5
library(ggplot2)
design<- seq(from=0, to=0.8, length=30)
load("fhnreplicatesparaminf.RDa")
...
pdf("fhnparaminf.pdf", height=4, width=8)
...
ggplot(data=df, aes(x=zetarange, y=mean)) + 
  ...
  geom_line() +
  geom_ribbon(aes(ymin=mean-sd, ymax=mean+sd), alpha = 0.2)
zetarange[which.max(df$mean)] #0.69
df$mean[which.max(df$mean)] # 1.95
