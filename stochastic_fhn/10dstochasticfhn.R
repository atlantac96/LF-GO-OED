...
xi_time <- function(t, zetas, T) {
  n <- length(zetas)           # number of segments
  seg_len <- T / n              # length of each segment
  seg_index <- min(floor(t / seg_len) + 1, n)  # find segment index
  return(zetas[seg_index])
}
...
utility_estimate <- function(zetas,
  ...
  if (any(is.na(zetas)) || length(zetas) != 10) return(NA_real_)
...
# --- Generalized 10-dim Bayesian Optimization Objective -------------------
BO_objective <- function(...) {
  zetas <- c(...)  # captures all input arguments as a numeric vector
  N_sim_per_eval <- 1000    # tune for speed/accuracy
  tol <- 0.06
  
  set.seed(123)  # reproducibility
  util <- utility_estimate(zetas, N_sim = N_sim_per_eval, tol = tol,
                           cur_for_zratio = 0.2)
  # rBayesianOptimization maximizes Score. Return very low value if invalid
...
# --- Bounds for 10 zetas -----------------------------------------------
n_dim <- 10
bounds <- setNames(lapply(1:n_dim, function(x) c(0, 0.8)), 
                   paste0("zeta", 1:n_dim))
...
#Round = 35	zeta1 = 0.003158671	zeta2 = 0.6206587	zeta3 = 0.6803862
#zeta4 = 0.5938132	zeta5 = 0.7972421	zeta6 = 0.2626193	
#zeta7 = 0.5402557	zeta8 = 0.4797727	zeta9 = 0.744107	zeta10 = 0.716713	
#Value = 0.451898 
