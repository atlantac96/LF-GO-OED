...
# ----------------------------
compute_utility_triple <- function(t1, t2, t3,
  ...
  idx <- pmin(pmax(round(c(t1, t2, t3)), 1), steps + 1)
...
test_val <- compute_utility_triple(2, 8, 14,
                                   priorabc, R_prior_mat, z_recov_prior,
                                   S_init, I_init, R_init, dt, steps,
...
bo_res <- BayesianOptimization(FUN = bo_fun,
                               bounds = bounds,
                               init_points = 30,   # initial random evals
                               n_iter = 50,       # sequential BO iterations
                               acq = "ucb",
                               kappa = 2.576,
                               verbose = TRUE)

print(bo_res)
#t1 = 18.88078	t2 = 5.460788	t3 = 3.380874	Value = 0.1995654 
