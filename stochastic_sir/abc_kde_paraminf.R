...
timetaken<-end-start
save(ut_matrix, file="utmatrixstochasticsirparaminf_infectedonly.RDa")

stopCluster(cluster)
library(ggplot2)
load("utmatrixstochasticsirparaminf_infectedonly.RDa")
...
pdf("sirparaminf_infectedonlyabc.pdf", height=4, width=8)
...
dev.off()
design[which.max(df$mean)] #0.3
df$mean[which.max(df$mean)] #1.797
